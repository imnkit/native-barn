...
class Interceptor extends \TemplateMonster\Megamenu\Block\Html\Topmenu implements \Magento\Framework\Interception\InterceptorInterface
{
     ...
     * {@inheritdoc}
     */
    public function renderMenu(\Magento\Framework\Data\Tree\Node $menuTree, $childrenWrapClass, $limit, $colBrakes = array())
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'renderMenu');
        if (!$pluginInfo) {
            return parent::renderMenu($menuTree, $childrenWrapClass, $limit, $colBrakes);
        } else {
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'renderMegamenuBlock');
        if (!$pluginInfo) {
            return parent::renderMegamenuBlock($category);
        } else {
            return $this->___callPlugins('renderMegamenuBlock', func_get_args(), $pluginInfo);
        }
    }
